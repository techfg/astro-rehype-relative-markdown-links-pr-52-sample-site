...
function getCurrentFileSlugDirPath(processingDetails) {
  ...
  */
  ...
  // TODO: Do we need to strip index.md here? .replace(/\/index$/, "");
  ...
  //return resolvedSlugPath === collectionDir ? resolvedSlugPath : path.dirname(resolvedSlugPath);
...
export function getRelativePathFromCurrentFileToDestination(processingDetails) {
  ...
  // TODO
  // "./" doesn't work but we can't use the collection name
  // I believe Astro adds a "slash" for index pages in collection roots
  // which makes sense because it would make it a directory
  // check this behavior with Astro and existing tests which should have a forced slash
  // on the end
  // see test "should transform root collection index.md paths with empty string custom slug"
  // on line 103 where the slash is at the end - this is the trick!
